...
export function createLovableAiGatewayRunIdFetch(initialRunId?: string) {
  ...
  return {
    fetch: async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      const body = init?.body ? JSON.stringify(JSON.parse(String(init.body))) : undefined;
      console.log("AI SDK request:", input, body);

      try {
        const response = await fetch(input, { ...init, headers });
        const cloned = response.clone();
        const text = await cloned.text();
        console.log("Gateway response:", response.status, text.slice(0, 500));
...
export function createLovableAiGatewayOpenAIProvider(
